...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="font-bold text-on-surface" style="font-size:22px; line-height:1.3;">Ubah Data Buku</h2>
            <p class="text-on-surface-variant" style="font-size:13px; margin-top:2px;">
                Memperbarui informasi katalog untuk
                <span style="font-weight:700; color:#00236f;">{{ $buku->judul }}</span>.
            </p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('buku.update', $buku->id) }}">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:18px;">
                    ...
                    <div style="grid-column: 1 / -1; display:flex; flex-direction:column; gap:5px;">
                        ...
                        <textarea name="deskripsi"
                                  ...
                                  onfocus="this.style.borderColor='#00236f'; this.style.boxShadow='0 0 0 3px rgba(0,35,111,0.1)';"
                                  onblur="this.style.borderColor='{{ $errors->has('deskripsi') ? '#ba1a1a' : '#c5c5d3' }}'; this.style.boxShadow='none';">{{ old('deskripsi', $buku->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p style="font-size:12px; color:#ba1a1a; font-weight:600;">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div style="margin-bottom: 28px;">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:18px;">
                    ...
                    <div style="display:flex; flex-direction:column; gap:5px;">
                        ...
                        <p style="font-size:11px; color:#757682;">
                            Sedang dipinjam: <strong style="color:#171c1f;">{{ $buku->stok_total - $buku->stok_tersedia }} eksemplar</strong>.
                            Stok tersedia akan dihitung otomatis.
                        </p>
                        @error('stok_total')
                            <p style="font-size:12px; color:#ba1a1a; font-weight:600; display:flex; align-items:center; gap:4px;">
                                <span class="material-symbols-outlined" style="font-size:14px; display:block;">error</span>
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; padding-top:20px; border-top:1.5px solid #eaeef2;">
                ...
                <form method="POST" action="{{ route('buku.destroy', $buku->id) }}"
                      onsubmit="return confirm('Hapus buku {{ addslashes($buku->judul) }}? Tindakan ini tidak dapat dibatalkan.')">
                    @csrf
                    @method('DELETE')
                ...
                <div style="display:flex; gap:10px;">
                    <a href="{{ route('buku.show', $buku->id) }}" class="btn-ghost">
